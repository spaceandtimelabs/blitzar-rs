    ...
    unsafe {
        INIT.call_once(|| {
            ...
            let backend = if cfg!(feature = "cpu") {
                proofs_gpu::SXT_BACKEND_CPU
            } else {
                proofs_gpu::SXT_BACKEND_GPU
            } as i32;

            // initializes the backend using the lower-level rust sys crate
            let config: proofs_gpu::sxt_config = proofs_gpu::sxt_config { backend };
...
fn to_sxt_ristretto_elements(num_sequences: usize) -> Vec<proofs_gpu::sxt_ristretto_element> {
    vec![
        proofs_gpu::sxt_ristretto_element {
            ristretto_bytes: [0_u8; 32]
        };
        num_sequences
    ]
}
...
fn to_sxt_ristretto_generators(
    generators: &[CompressedRistretto],
) -> Vec<proofs_gpu::sxt_ristretto_element> {
    (0..generators.len())
        .map(|i| proofs_gpu::sxt_ristretto_element {
            ristretto_bytes: generators[i].to_bytes(),
        })
        .collect()
}
...
fn to_sxt_descriptors<T: private::Descriptor>(
    data: &[T],
) -> (Vec<proofs_gpu::sxt_sequence_descriptor>, usize) {
    let mut longest_row: usize = 0;
    let num_sequences = data.len();
    let cbinding_descriptors: Vec<proofs_gpu::sxt_sequence_descriptor> = (0..num_sequences)
        .map(|i| {
...
fn to_pedersen_commitments(
    ...
    for i in 0..num_sequences {
        commitments[i] =
            CompressedRistretto::from_slice(&sxt_ristretto_elements[i].ristretto_bytes);
    }
}
